...
model = nn.Sequential(
    ...
    nn.Flatten(),
    nn.Linear(4 * 14 * 14, model_classes_number)
).to(device)
...
class CustomCNN(nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=2)
        self.flatten = nn.Flatten()
        self.fc1 = nn.Linear(in_features=4 * 14 * 14, out_features=n_classes)
...
custom_model = CustomCNN(model_classes_number).to(device)
...
for t in range(epochs):
    tr_loss = train(train_data_loader, custom_model, loss_fn, optimizer, device)
    val_loss, pred = evaluate(x_val, y_val, custom_model, loss_fn, device)
