...
# 모델 생성 함수
def make_model(node, data_size, classes):
    """
    :param node:
    :param epoch:
    :param data_size:
    ...
    """
    model = nn.Sequential(
        nn.Conv2d(in_channels=1, out_channels=node, kernel_size=3, padding=1, stride=1),
        nn.ReLU(),
        nn.MaxPool2d(kernel_size=2, stride=2),
        nn.Flatten(),
        nn.Linear(in_features=node * (data_size // 2) * (data_size // 2), out_features=classes)
    ).to(device)
    return model
...
node_number = 4
# 노드에 대한 숫자를 변경 -> 사용자 입력
# node_number = int(input("노드에 대한 숫자를 입력 하세요 >> "))

# 모델링
simple_model = nn.Sequential(
    nn.Conv2d(in_channels=1, out_channels=node_number, kernel_size=3, stride=1, padding=1),
    nn.ReLU(),
    nn.MaxPool2d(kernel_size=2, stride=2),
    nn.Flatten(),
...
# 모델 생성 및 학습 하는 함수
def make_model_train(node, data_size, epoch=10):
    """
    :param node:
    :param data_size:
    :param classes:
    ...
    :return:
    """
    model = make_model(node, data_size, len(classes))
...
for n in range(5, 101, 5):
    acc.append(make_model_train(n, 28))
    print(n)
